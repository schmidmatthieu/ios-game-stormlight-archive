// ─── Entity Spawner (enemies, NPCs, loot, exits) ───────────────
...
import type { Zone, Enemy } from '../data/types';
import type { EnemyInstance, NPCInstance, LootInstance } from './ZoneTypes';

// ─── Enemies ─────────────────────────────────────────────────

export function spawnEnemies(zone: Zone, worldContainer: Container): EnemyInstance[] {
...
export function spawnNPCs(zone: Zone, worldContainer: Container): NPCInstance[] {
...
export function spawnLootPoints(zone: Zone, worldContainer: Container, onCollect: (id: string) => void): LootInstance[] {
...
export function renderExits(zone: Zone, worldContainer: Container): void {
  for (const conn of zone.connections) {
    ...
    portal.poly([
      { x: 0, y: -24 }, { x: 14, y: -10 }, { x: 0, y: 4 }, { x: -14, y: -10 },
